...
def showcontourf(mat,D,cmap=plt.cm.get_cmap('jet'),fsize=(12,12),vmin=0, vmax=100, timestep1 =0, name = str(11)):
    plt.clf()
    levels = cp.arange(vmin,vmax,1)
    x=cp.linspace(D[0],D[1],mat.shape[1])
    y=cp.linspace(D[2],D[3],mat.shape[0])
    X,Y=cp.meshgrid(x,y)
    z_max = cp.max(mat)
    i_max,j_max = cp.where(mat==z_max)[0][0], cp.where(mat==z_max)[1][0]
